...
-- 출력을 위해 한번은 실행해야됨 (이건 주석을 옆에 하면안됨)
/*
SET SERVEROUTPUT ON;
...
BEGIN
    SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY, DEPT_TITLE
     ...
     DBMS_OUTPUT.PUT_LIN(EID || ', ' || ENAME || ', ' || JCODE || ', ' || SAL || ', ' || DTITLE);
...
-- 3) IF 조건식1 THEN 실행내용1 ELSIF 조건식2 THEN 실행내용2 ... [ELSE 실행내용N] END IF; (IF-ELSE IF)
DECLARE
    SCORE NUMBER;
    GRADE VARCHAR2(10);
BEGIN
