...
GRANT CREATE VIEW TO BR; --> 관리자 계정에서 실행 해야됨
...
SELECT EMP_NAME, SALARY, BONUS -- BONUS 오류 OR REPLACE 이용하여 해결
FROM VW_EMPLOYEE
WHERE NATIONAL_NAME = '러시아';

-- [참고] USER_VIEWS : 현재 이 사용자가 가지고 있는 뷰 객체에 대한 정보를 조회할 수 있는 시스템 테이블
...
/*
    * 뷰 컬럼에 별칭 부여
      서브쿼리의 SELECT절에 산술연산식, 함수식을 기술했을 경우 반드시 별칭 지정해야됨
*/
...
-- 아래와 같은 방법도 가능 (단, 모든 컬럼에 대한 별칭을 다 작성)
...
SELECT * FROM JOB;    -- 베이스 테이블 (실제 데이터가 담겨있는)

-- 뷰를 통해서 INSERT
INSERT INTO VW_JOB VALUES('J8', '인턴'); -- JOB에도 INSERT
...
/*
    * DML로 조작이 불가능한 경우가 더 많음
    ...
    2) 뷰에 정의되어있지 않은 컬럼 중에 베이스테이블상에 NOT NULL제약조건이 걸려 있는 경우 
...
SELECT * FROM VW_JOB; -- 한 개의 컬럼
SELECT * FROM JOB; -- 두 개의 컬럼

-- INSERT (오류) -- 뷰에는 한 개의 컬럼밖에 없기 때문
...
-- 2) 뷰에 정의되어있지 않은 컬럼 중에 베이스테이블상에 NOT NULL제약조건이 걸려있는 경우
CREATE OR REPLACE VIEW VW_JOB
AS SELECT JOB_NAME
     FROM JOB;
...
-- INSERT (오류)
INSERT INTO VW_EMP_SAL VALUES(400, '홍길동', 40000000, 1); -- virtual column not allowed here
INSERT INTO VW_EMP_SAL(EMP_ID, EMP_NAME) VALUES(400, '홍길동'); -- cannot insert NULL into ("BR"."EMPLOYEE"."EMP_NO")

-- UPDATE
-- 200번 사원의 연봉을 8000만원으로 
UPDATE VW_EMP_SAL
   SET 연봉 = 80000000 
 WHERE EMP_ID = 200; -- 오류 : 수식이 있을 뿐 연봉의 값이 없다 
...
-- INSERT (오류) -- EMPLOYEE 테이블에 적절한 값이 아님
INSERT INTO VW_GROUPDEPT VALUES('D3', 8000000, 4000000);
-- UPDATE (오류)  -- EMPLOYEE 테이블에 적절한 값이 아님
 UPDATE VW_GROUPDEPT 
   SET 평균 = 3000000
 WHERE DEPT_CODE = 'D1';
-- DELETE (오류)  -- EMPLOYEE 테이블에 적절한 값이 아님
...
UPDATE VW_JOINEMP
   SET EMP_NAME = '홍길동'
 WHERE DEPT_TITLE = '총무부'; -- 성공 : 조건으로 쓰면 가능
...
-- 200번 사원의 급여를 200만원으로 (서브쿼리의 조건에 부합되지 않는 값으로 변경)
UPDATE VW_EMP
  SET SALARY = 2000000
 WHERE EMP_ID = 200; -- 7명 조회
...
UPDATE VW_EMP
   SET SALARY = 4000000
 WHERE EMP_ID = 200; -- 성공 8명 조회

SELECT * FROM EMPLOYEE; -- EMPLOYEE에서도 변경됨
